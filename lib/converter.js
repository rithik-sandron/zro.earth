import { un, Hline, Olist, Ulist, UlistTask, 
    ...
    accordion, accord, codeHighlight } from './regex';
                    ...
                    case table.test(line):
                        ...
                        while (i < len) {
                            ...
                            for (let k = 0; k < cells.length; k++) {
                                ...
                                dataHtml += isHeading ? (`<th>${cells[k].trim()}</th>`) :
                                    cells[k].trim() === '' ?
                                        (`<td class='empty-row'>${cells[k].trim()}</td>`) : (`<td>${cells[k].trim()}</td>`);
                            }
                            dataHtml += ('</tr>');
                            isHeading = false;
                            i++;
